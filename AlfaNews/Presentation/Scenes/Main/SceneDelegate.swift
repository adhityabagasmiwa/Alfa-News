...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    @objc func handleNetworkStatusChange(_ notification: Notification) {
        if let status = notification.object as? NetworkReachabilityManager.NetworkReachabilityStatus {
            switch status {
                case .notReachable:
                    showAlertDialog(title: .connection, message: .connection)
                default:
                    debugPrint("[LOG] - Unknown network status")
            }
        }
    }
    
    func showAlertDialog(title: CommonErrorTitle, message: CommonErrorDesc) {
        let alertController = UIAlertController(title: title.rawValue, message: message.rawValue, preferredStyle: UIAlertController.Style.alert)
